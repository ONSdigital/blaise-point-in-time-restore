...
class TestOrmFunctionality:
    ...
    def mock_source_database(self, mock_source_session) -> DatabaseService:
        return DatabaseService(mock_source_session)

    @pytest.fixture()
    def mock_destination_database(self, mock_destination_session) -> DatabaseService:
        return DatabaseService(mock_destination_session)
    ...
    def table(self) -> Table:
        tables = TableFactory().get_table_models(self.questionnaire_name)
        return tables[0]

    @pytest.fixture()
    def mock_source_records(self, table: Table) -> [Table]:
    ...
    def test_copy_table_data_copies_source_database_records_to_destination_database(self,
        ...
        # act
        service_under_test.copy_table_data(table)
